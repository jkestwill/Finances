...
class GoodsListViewModel @Inject constructor(
    ...
    private var _goodsStateFlow = MutableStateFlow<State<List<Goods>>>(State.None)
    val goodsStateFlow: Flow<State<List<Goods>>> get() = _goodsStateFlow


    fun getAllGoods() {
        viewModelScope.launch {
            goodsRepository.getAllFromDatabase().map {
                it.toState()
            }.collectLatest {
