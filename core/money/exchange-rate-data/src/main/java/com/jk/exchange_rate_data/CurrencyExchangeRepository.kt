...
class CurrencyExchangeRepository(
    ...
    fun getExchangeRate(
        ...
        mergeStrategy: ApiRequestMergeStrategy<ExchangeRate>
    ...
    private fun fetchFromApi(
        ...
        val result = flow<ExchangeRateApiRequest> {
            ...
            val exRelation = ExchangeRateRelation(
                ExchangeRateEntity(
                    id = "${exRate.rate}${exRate.currencyOut}${exRate.currencyIn}${exRate.scale}${exRate.date}".sha256(),
    ...
    private fun getFromDatabase(exchangeRateRequestParams: ExchangeRateRequestParams): Flow<ApiRequest<ExchangeRate>> {
        val result: Flow<ApiRequest<ExchangeRate>> = flow<ExchangeRateRelation> {
            emit(
                exchangeRateDao.getList(
                    exchangeRateRequestParams.currencyIn,
                    exchangeRateRequestParams.currencyOut,
                    exchangeRateRequestParams.date
                )
            )
        }.map { list ->
